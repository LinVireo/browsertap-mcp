...
While connected, BrowserTap keeps tab identifiers, URLs, titles and connection metadata so it can route commands. Fixed content scripts run on matching pages for connection indication and dialog handling. The extension holds access to all sites covered by its manifest; that access is not a fresh per-command permission.
...
The extension's built-in bridge transport connects to loopback on the same computer. Results are returned to the configured MCP client. If that client uses a remote model or service, it may send task data to that provider under its own terms. Requested page operations can also contact websites or download endpoints.
