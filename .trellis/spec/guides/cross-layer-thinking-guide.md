...
## Protected Browser UI Boundary

Chrome Web Store dashboards and other browser-internal or protected pages may
reject page-script/CDP access even when the page is visible. Treat this as a
separate external UI boundary rather than a bridge failure.

- Bind the operation to one stable item identifier and an exact URL pattern.
- Confirm the foreground window and document are unique before writing.
- Use the platform's native UI Automation surface for controls and file dialogs
  when page scripting is refused.
- Read every changed field back and stop on an ambiguous control, navigation, or
  unknown native-dialog result.
- Keep irreversible actions such as `Submit for review` behind an explicit human
  gate.

The wrong pattern is to retry a denied page script against whichever tab is
currently selected. The correct pattern is to verify the protected document,
perform one bounded native action, and record the read-back evidence.
