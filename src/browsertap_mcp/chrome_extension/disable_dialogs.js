...
function manageDialogScope(action, scope) {
  ...
  const controller = {
    version: 1,
    enter(value) {
      const entry = { token: String(value.token), policy: value.policy, deadline: value.deadline, records: [] };
      scopes.push(entry);
      refresh();
      return {
