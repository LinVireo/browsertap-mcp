...
- 优先 CSS 或结构化 locator：`css`、`role+name`、`text`、`label`。
  locator 内 `selector` 是 `css` 的别名；`frame` 可逐层进入同源和跨域 iframe，
  `shadow` 可进入开放 Shadow DOM。
  role 定位排除隐藏、`aria-hidden` 和 inert 控件；可见 disabled 控件仍可满足查询，
  输入前需确认可交互。CSS 查询仍按 DOM 存在性判断。
- 零匹配、歧义、非法 CSS、不可交互分别按 `not_found`、`ambiguous`、
  `invalid_selector`、`not_interactable` 处理；这些拒绝不派发输入。
  closed shadow root 需换可访问的定位范围。
- selector click 会命中测试。可滚动到目标时返回 `scrolled_into_view`；
  ...
  通过时有 `hit_verified=true`。iframe 内点击不自动滚动，并检查每层父 frame 的遮挡。
- 普通坐标使用顶层视口 **CSS 像素**。iframe 内部点位可用
  `page_click(selector={"frame":[...],"x":20,"y":30}, session_id=...)`，
  由 BTAP 换算为顶层坐标。frame 或其祖先带非恒等 transform、zoom 或 perspective
  时返回 `unsupported_frame_transform`；查询和键盘输入仍可用。
  点位模式不做元素命中测试。
...
SPA 先读 `scan_page.render`，再用 `wait_for` 等业务控件；正文就绪不证明目标字段已就绪。
内置就绪探测超时会释放自身标签占用；缺少 `render` 是状态未知，继续等目标控件。
同名隐藏模板用 role/name 定位；两个可见匹配仍是 `ambiguous`，需缩小范围。
同源、同进程跨域和 OOPIF 使用相同的高层定位接口：

```python
field = {"frame": ["#outer", "#inner"], "css": "#control"}
wait_for(selector=field, session_id=session_id)
page_type("example", selector=field, clear=True, session_id=session_id)
page_click(selector={"frame": ["#outer", "#inner"], "role": "button", "name": "Search"}, session_id=session_id)
```

每层 frame selector 必须唯一；路径按当前 DOM 元素解析。一次调用内绑定实际文档和元素，
每条输入前检查连接、焦点或点击位置。绑定后导航或替换返回 `stale_frame`。
新调用可以定位导航后的文档，但先检查 `input_dispatched`：部分输入或结果未知时，
检查原操作和页面状态，不重放整段输入。旧扩展返回 `stale_extension`，按
`next_action=reload_extension` 手工 Reload。
`wait_for(gone=True)` 仅在目标不存在时成立，歧义和失效绑定不算消失。
iframe 等待返回未完成 `operation_id` 时可能仍有占用；按 `reservation_held` 处理并领取原结果。
