...
def _persist_token(path: Path, token: str) -> str:
    ...
    try:
        ...
        # POSIX O_EXCL publishes before writing; Windows reads wait for the
        # creator's exclusive handle. Converge on the winner's complete value
        # instead of inventing a different token that would split the clients.
        for _ in range(20):
