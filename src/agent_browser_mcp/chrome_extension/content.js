...
(function(){
  ...
  const stale = document.getElementById('abm-indicator');
  ...
  const TEXT_CONNECTED = message('indicatorConnected', 'ABM: connected');
  const TEXT_DISCONNECTED = message('indicatorDisconnected', 'ABM: disconnected');
  d.innerText=message('indicatorChecking', 'ABM: checking');
  ...
  function paint(ok){
    try {
      if (document.getElementById('abm-indicator') !== d) return;
  ...
  try {
    ...
    const applyStored = (stored) => {
      if (document.getElementById('abm-indicator') !== d) return;
      // Read both key names: the popup migrates the pre-ABM preference, but a
      ...
      applyVisibility(stored.abm_indicator_visible !== undefined
        ? stored.abm_indicator_visible
    ...
    const pendingRead = chrome.storage.local.get(
      ['abm_indicator_visible', 'tmwd_indicator_visible'], applyStored,
    );
    ...
    chrome.storage.onChanged.addListener((changes, areaName)=>{
      ...
      const change = changes.abm_indicator_visible || changes.tmwd_indicator_visible;
  ...
  const receiveStatus = (status)=>{
    if (status && status.type === 'abm_status') paint(Boolean(status.ws));
  };
  try {
    chrome.runtime.onMessage.addListener((msg)=>{ receiveStatus(msg); });
    const port = chrome.runtime.connect({ name: 'abm_keepalive' });
    port.onMessage.addListener(receiveStatus);
    port.postMessage({ type: 'abm_status_request' });
  } catch(_) { paint(false); }
})();
