...
def test_non_answering_builder_does_not_serialize_a_dialog_helper(policy):
    scope = None if policy is None else {"token": "t1", "policy": policy}
    outcome = _dialog_scope_builder_harness(
        f"""
process.stdout.write(JSON.stringify({{
  script: buildExecScript('1', 'return null;', {json.dumps(scope)}),
}}));
"""
    )
    assert "const _btapHasDialogScope = false;" in outcome["script"]
    assert "const _btapManageDialogScope = null;" in outcome["script"]
...
const installation = vm.runInContext(`(${{globalThis.manageDialogScope.toString()}})('install',
  ${{JSON.stringify({{...scope, deadline: Date.now() + dialogScopeWindowMs(scope)}})}})`, scoped.context);
if (installation?.__btap_dialog_scope_installed !== true) throw new Error('scope installation not confirmed');
...
def test_exec_injection_reaches_every_frame_but_returns_the_top_one():
    ...
    assert "buildPageScript(data.code, frameDialogScope)" in inject
    ...
    assert "return eval(sub);" not in inject
    assert "entry?.result?.__btap_top_frame_result === true" in inject
    preparation = source[source.index("      if (answeringScope) {"):source.index("      const inject = async () => {")]
    assert "func: globalThis.manageDialogScope" in preparation
    assert "args: ['install', frameDialogScope]" in preparation
