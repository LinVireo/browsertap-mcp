...
def run_node(source: str) -> None:
    program = (
        ...
        "(async () => {\n" + source
        + "\nprocess.stdout.write('BTAP_DIALOG_TEST_DONE\\n');\n})().catch(error => {\n"
    ...
    # Fake page/worker timers do not keep Node alive. An unresolved command must
    # fail instead of exiting 0 before the assertions after its await run.
    assert completed.stdout == "BTAP_DIALOG_TEST_DONE\n", "scenario did not complete"
