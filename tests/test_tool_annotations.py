...
@pytest.mark.anyio
async def test_setup_diagnosis_acl_hardening_is_an_idempotent_effect():
    advertised = {tool.name: tool for tool in await S.mcp.list_tools()}
    hints = advertised["get_setup_status"].annotations
    ...
    assert S.TOOL_CAPABILITIES["get_setup_status"]["side_effect"] == "mixed"
