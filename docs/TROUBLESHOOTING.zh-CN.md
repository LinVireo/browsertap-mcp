...
### 导航在 `Page.enable` 阶段失败

0.5.4 之前，即使调用总超时仍有余量，导航初始化也可能在两次各 2.5 秒的
`Page.enable` 尝试后提前失败。完全本地页面的主线程忙等待已复现此问题，无需外部网络
延迟即可触发。0.5.4 将 debugger attach 限制在同一 deadline 内，并让唯一一次初始化
重试使用调用剩余时间。`Page.navigate` 仍最多派发一次；初始化失败报告 `dispatched=false`。

原始偶发现场的具体触发原因仍未确认，此修复不代表其他 JavaScript 读取或对话框清理超时
已经定位。出错后先核对操作回执和目标状态，再决定是否重试有副作用的调用；后续调用成功
本身不能证明问题已修复。
